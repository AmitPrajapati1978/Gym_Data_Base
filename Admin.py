...
#         FROM members m
#         JOIN membership_plans p ON m.plan_id = p.plan_id
#     WHERE strftime('%Y', m.expiration_date) = '{selected_year}'
#     AND strftime('%m', m.expiration_date) = '{selected_month:02d}';
#     """
...
def add_member(name, join_date, plan_id):
    ...
    cursor.execute("SELECT duration_in_days FROM membership_plans WHERE plan_id = ?", (plan_id,))
    result = cursor.fetchone()

    if result:
        duration = result[0]
        # Insert new member with expiration date
        cursor.execute("""
            INSERT INTO members (name, join_date, plan_id, expiration_date)
            VALUES (?, ?, ?, DATE(?, '+' || ? || ' days'))
        """, (name, join_date, plan_id, join_date, duration))
        db.commit()
        st.success("✅ Member added successfully!")
    else:
...
with tab6:
    ...
    if df_perf.empty:
        ...
        # Show bar chart
        st.bar_chart(df_perf.set_index("Trainer")["Total Events"])

        # Show data table
        st.dataframe(df_perf, use_container_width=True)

        # 🔥 Highlight top trainer
        top_trainer = df_perf.iloc[0]
        st.success(
            f"🏆 **{top_trainer['Trainer']}** is leading with **{top_trainer['Total Events']} events**!"
        )

        # 💸 Suggest a raise if doing amazing
        if top_trainer["Total Events"] >= 10:
            st.markdown(f"💰 Consider giving **{top_trainer['Trainer']}** a raise or bonus!")
        elif top_trainer["Total Events"] >= 5:
            st.markdown(f"👏 **{top_trainer['Trainer']}** is doing well — keep an eye on their performance.")
        else:
            st.markdown("📊 Performance is still low across the board. Let's book more events!")


# === TAB 7: EVENT ATTENDANCE ===
with tab7:
    st.markdown("### 🏆 Top 10 Most Attended Events")
    ...
    if df_top.empty:
        st.info("📭 No attendance data available. Add some attendance records first!")
    else:
        st.subheader("📈 Attendance Leaderboard")
        st.bar_chart(df_top.set_index("Event"))
        st.dataframe(df_top, use_container_width=True)

        # Optional: Show top winner
        top_event = df_top.iloc[0]
        st.success(f"🏅 **{top_event['Event']}** is the most attended with **{top_event['Attendance Count']} check-ins**!")
